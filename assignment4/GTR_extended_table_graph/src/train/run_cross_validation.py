    ...
    with torch.no_grad():
        for qid in query_id_list:
            ...
            for (tid, rel) in qtrels[qid].items():
                table = tables_with_query[qid]["tables"][tid]
                dgl_graph = tables_with_query[qid]["tables"][tid]["dgl_graph"].to("cuda")
                node_features = tables_with_query[qid]["tables"][tid]["node_features"].to("cuda")
...
def cross_validation(config):
    ...
    global queries, tables, qtrels, tables_with_query
    queries = {}
    ...
    tables_with_query = process_tables(tables, constructor, queries)
